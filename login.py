...
def teleport(li,id1):
	for i in li :
		if i != "teleport-bundle.properties" :
			...
			file1.close()
	gconf_id = kap.json()
	new_gconf_id = gconf_id.get('g_conf_id')
	return (new_gconf_id)
...
def read():
	try:
		...
		for i in csv_file:
			...
			count =  count + 1
			print ("Name = ", gconfid)
			n_g_id = extract(gconfid, date, user, user_id)
			with open('data.csv', 'a') as csvfile:
				fieldnames = ['old_id', 'new_id']
				writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
				writer.writerow({'old_id': old_gconfid, 'new_id': n_g_id})
				print ('old_id = ',old_gconfid,'|| new_id = ', n_g_id)	
